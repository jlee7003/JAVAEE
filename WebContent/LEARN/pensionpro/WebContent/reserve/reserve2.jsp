  ...
  <tr align=center> <!-- 주 -->
    ...
    %>
        <td valign=top align=left> <!-- 날짜 출력되는 td -->
          ...
          sql=sql+" and out_date > '"+nowday+"' and bang='chan'";
          //out.println(sql);
          rs=stmt.executeQuery(sql);
          if(rs.next()) // 방이 현재날짜에 예약되어있다..
          {
          %>
           <p align=center>  차니1호(불가)  </p>
          <%
          }
          else
          {
          %> 
           <p align=center> <a href="input.jsp?bang=chan&y=<%=y%>&m=<%=m%>&d=<%=day%>"> 차니1호(가능) </a> </p>
          <%
          }
          %>  
           <%
          nowday=y+"-"+m+"-"+day;  // y,m,day => 2019-07-02
          sql="select * from reserve where in_date <= '"+nowday+"'";
          sql=sql+" and out_date > '"+nowday+"' and bang='chan2'";
          //out.println(sql);
          rs=stmt.executeQuery(sql);
          if(rs.next()) // 방이 현재날짜에 예약되어있다..
          {
          %>
           <p align=center>  차니2호(불가)  </p>
          <%
          }
          else
          {
          %> 
           <p align=center> <a href="input.jsp?bang=chan2&y=<%=y%>&m=<%=m%>&d=<%=day%>"> 차니2호(가능) </a> </p>
          <%
          }
          %>  

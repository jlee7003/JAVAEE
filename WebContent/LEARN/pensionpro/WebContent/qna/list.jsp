...
<jsp:include page="comm_menu.jsp"></jsp:include>
 ...
 <div id=right>
  <div id=qna>묻고 답하기</div> 
  <table align=center width=800 border=0 cellspacing=0>
   <tr id=first height=30 align=center>
     <td> 이름 </td>
     <td> 제목 </td>
     <td> 조회수 </td>
     <td> 작성일 </td>
<!--      <td> ref </td>
   ...
   <tr height=20 id=second>
     ...
     <td>
      ...
      %>
      <%
        if(rs.getInt("depth") !=1 ) // 답글일 경우
        {
      %>
      <img src=1.png width=13>
      <%
        }
      %>
      <a href="rnum_add.jsp?id=<%=rs.getString("id")%>&page=<%=Page%>"> <%=rs.getString("title")%> </a> </td>

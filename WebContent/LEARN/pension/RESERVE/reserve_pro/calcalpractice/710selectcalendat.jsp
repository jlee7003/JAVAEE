...
{
	var y=document.all.lang.value;
    var m=document.all.month.value;
	location="710selectcalendat.jsp?yy="+y+"&mm="+m;
}
  ...
  <select id="id-lang" name="lang" onchange="chageLangSelect()">
    ...
    for(int a=2010;a<2020;a++)
    {
    	if(a==y)
			...
			sel1="";
    %>
  <option value=<%=a%> <%=sel1%>><%=a%></option>
  <%
   }
  %>
   </select>
        <select name=month onchange=chageLangSelect()>
    <%
    int sss=0;
    String sel2="";
    for(int s=1;s<=12;s++)
